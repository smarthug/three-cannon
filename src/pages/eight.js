...
// import {install} from './ExtendMaterial'
let hosuk = {};
...
export default function Main() {
  ...
  function Init() {
    ...
    // material
    var material = hosuk.extendMaterial(THREE.MeshStandardMaterial, {
...
export function install(THREE) {
  ...
  function extend(source, object) {
    ...
    if (material.isCustomMaterial) material["is" + name] = true;
    ...
    for (let name in map) {
      ...
      if (value instanceof Object) {
        if (THREE.ShaderChunk[name] === undefined) {
          ...
          chunk = chunk.replace(
            "#include <" + name + ">",
            applyPatches(THREE.ShaderChunk[name], value)
          );
        }
      } else {
        if (name[0] === "@") {
  ...
  function applyConstants(name, uniform, defines, object, instance) {
    ...
    //|| flag.not !== value
    if (flag !== undefined && flag.not === undefined)
